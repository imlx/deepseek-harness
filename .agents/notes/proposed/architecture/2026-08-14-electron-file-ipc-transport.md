...
3. **An Electron main-process shell (`apps/electron`).** The main process boots a dsh profile in-process over the public `@deepseek-ai/dsh-app-boot` API, disables every port-binding and browser-graph row through an overlay patch, and bridges the gateway: unary calls forward to `toFetchHandler(apiProxy)`, and each downlink stream iterates `apiProxy.events` in-process and pushes frames to the renderer over IPC. No `webserver`, no browser trust fence — the main process is the same application, so the HTTP threat model does not apply.
